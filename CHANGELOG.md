...
- A default on the `_generate_next_value_` of an `enum.Enum` subclass is reported but retained. `auto()` calls it implicitly with the four arguments the enumeration machinery has, so removing the default made creating the member raise `TypeError`.
