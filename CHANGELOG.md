...
- `import package.module` makes `package.module.Base` the submodule's class, over a same-named class the package initializer defines. The initializer's class was winning, and inherited calls were rewritten with defaults the real base never had. A class written here above the subclass still takes the name back off the import, and everything spelled under it goes with the name — as does a class put behind that name by an assignment such as `package = Local`. An import written below either of those hands the name back to the module again, and a subclass under it is built on the module's class. An import this run cannot follow takes the name back just as surely, so the subclass's ancestry is left unknown rather than resolved against the class of that spelling written above it. Only an import certain to run counts: one written in a branch that may not be taken leaves the class standing.
- A default on the `__init__` of an `enum.Enum` subclass is reported but retained. Assigning a member calls it implicitly while the class is created, so removing the default made the class itself raise `TypeError`, and there is no written call site to keep in step: the calls are the member assignments the interpreter makes. A class written here that inherits from another enum of this file's own counts as one however far down the chain it is, including where the base is a class beside it in the same class body.
