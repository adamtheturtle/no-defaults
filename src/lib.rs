...
/// The value a binding statement assigns and the targets it assigns it to.
///
/// An annotated binding names a class just as a plain one does, so a subclass
/// reaching the original through it is linked the same way. A bare annotation
/// assigns nothing and binds nothing.
fn assigned_binding(statement: &Stmt) -> Option<(&Expr, &[Expr])> {
    match statement {
...
fn index_method_bases(
    ...
    // identity a spelling resolves to carries the enclosing class's name, or
    // the lexical scope holding it when no class does.
    ...
    for statement in statements {
        ...
        match statement {
            ...
            Stmt::Assign(_) | Stmt::AnnAssign(_) => {
                let Some((value, targets)) = assigned_binding(statement) else {
                    continue;
                };
