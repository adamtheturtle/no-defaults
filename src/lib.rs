...
fn method_base_identity(
    ...
    match expression {
        ...
        Expr::Attribute(attribute) => {
            ...
            if let Some((identity, _)) = dotted_name(expression)
                .and_then(|qualified| local_classes.get(&qualified))
            ...
            // Importing a submodule binds it on its package, so a prefix that
            // names one reaches that file and not whatever the package
            // initializer happened to write under the same name. Reading the
            // components as nested classes of the initializer instead would
            // answer with a class the subclass was never built on.
            if let Some(Binding::Module(file)) =
                dotted_name(&attribute.value).and_then(|module| bindings.get(&module))
            {
            ...
            // left alone.
            let (file, parent) = method_base_identity(
                &attribute.value,
                importer,
                bindings,
                local_classes,
...
",
    ...
    fn dotted_submodules_win_over_package_nested_classes() -> Result<(), String> {
        // Importing `package.module` binds the module on its package, so the
        // base is the class that file writes. Reading the components as
        // nested classes of the initializer would answer with the `Base` the
        // submodule import replaced, and the call would carry its default.
        let directory = tempfile::tempdir().map_err(|error| error.to_string())?;
        let package = directory.path().join("package");
        std::fs::create_dir(&package).map_err(|error| error.to_string())?;
    ...
    fn imported_metaclass_uncertainty_propagates_through_local_bases() {
        // `Middle` writes no fields of its own, so nothing here marks it, but
        // whatever metaclass builds `Parent` builds it and `Child` beneath it
        // alike. The default behind the construction has to stay put.
