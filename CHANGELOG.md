...
- A default on the `exec_module` of a class is reported but retained. `importlib` runs a module by calling the loader itself, handing it the module and nothing else, so a parameter beside it is only ever filled by its default, and removing it left the import raising `TypeError` from inside `importlib._bootstrap` with no written call site for the fixer to keep in step.
- A default on the `persistent_id` of a class is reported but retained. `pickle` asks a pickler whether each object is persistent by calling that hook with the object alone, so a parameter beside it is only ever filled by its default, and removing it left `dump` raising `TypeError` from inside `pickle` with no written call site for the fixer to keep in step.
