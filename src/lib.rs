...
impl Definitions {
    ...
    /// Every removed default a class of unsettled ancestry could inherit.
    ///
    /// Which of the competing definitions runs is not known here, so a call
    /// through one of them resolves to nothing and cannot be rewritten. The
    /// defaults behind all of them have to stay.
    ...
    fn linearized_mro(
        ...
        // Competing definitions leave no one ancestry to linearize, and the
        // bases recorded for the class are only the last suite's.
...
/// Where a statement list is walked from.
///
/// A control-flow suite opens no scope of its own, so the classes and aliases
/// the statements around it left standing are still what a base written in it
/// names. A class body and a function body do open one, and are entered with
/// nothing carried in.
#[derive(Clone, Copy)]
struct IndexScope<'a> {
    namespace: ClassNamespace<'a>,
    lexical_classes: LexicalClasses<'a>,
    /// The classes the scope holding a suite defines.
    local_classes: &'a BTreeMap<String, (String, TextSize)>,
    /// The names that scope has bound to a class before the suite.
    aliases: &'a BTreeMap<String, (PathBuf, String)>,
}

impl<'a> IndexScope<'a> {
    /// A scope entered afresh, holding none of the names around it.
    fn entered(namespace: ClassNamespace<'a>, lexical_classes: LexicalClasses<'a>) -> Self {
        static NO_CLASSES: BTreeMap<String, (String, TextSize)> = BTreeMap::new();
        static NO_ALIASES: BTreeMap<String, (PathBuf, String)> = BTreeMap::new();
        Self {
            namespace,
            lexical_classes,
            local_classes: &NO_CLASSES,
            aliases: &NO_ALIASES,
        }
    }
}
...
/// The classes a base written among these statements can name.
///
/// A suite is walked on its own, so the classes written beside it come in from
/// the scope that holds it. Without them a class in the suite would find no
/// base outside it, and the call it inherits would be left alone after the
/// default behind it was stripped. A class written here holds the spelling
/// over one of the same name further out.
fn visible_local_classes(
    statements: &[Stmt],
    scope: IndexScope<'_>,
) -> BTreeMap<String, (String, TextSize)> {
    let mut local_classes = local_class_identities(statements, scope.namespace);
    for (spelling, identity) in scope.local_classes {
        local_classes
...
fn index_method_bases(
    ...
    let local_classes = visible_local_classes(statements, scope);
...
fn index_control_flow_method_bases(
    ...
    for suite in control_flow_suites(statement) {
        ...
        index_method_bases(suite, importer, known, bindings, scope, definitions);
        // Only one suite of a statement runs, so a class two of them define
        // with different bases has no ancestry this pass can settle. The last
        // definition walked would otherwise stand for all of them and resolve
        // inherited calls against a class the running one never had.
...
fn method_base_identity(
    ...
    match expression {
        ...
        Expr::Attribute(attribute) => {
            // A dotted submodule binding is more specific than reading the
            // same components as nested classes in the package initializer,
            // which is what the module lookup below would otherwise settle
            // for only after those classes had answered.
...
",
    ...
    fn competing_conditional_class_bases_retain_inherited_defaults() -> Result<(), String> {
        ...
        let mut definitions = Definitions::default();
        let mut outside_functions = BTreeSet::new();
        collect_indexed_class_names(module.suite(), None, &mut outside_functions);
        index_method_bases(
            module.suite(),
            Path::new("fixture.py"),
            &BTreeSet::new(),
