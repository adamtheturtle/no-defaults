...
- Two definitions of one class in different branches that name different bases keep the defaults of every method either ancestry holds. Which base the class ends up with is not knowable, and `--fix` was resolving inherited calls against whichever branch was read last and stripping the defaults behind the others. A method the class itself, or a subclass of it, defines is still resolved: it does not depend on which ancestry ran.
- A name that competing branches bind to different classes leaves the ancestry of a subclass built on it unsettled, as two definitions of the class itself already do. After `if …: Alias = First` / `else: Alias = Second`, a `class Child(Alias)` was resolved against whichever assignment was read last and the other candidate's defaults were stripped. An assignment outside the branches settles the name again.
...
- A class name in an alias such as `Alias = Base` is looked up the way Python looks it up: the scope holding it first, then the functions and the module around it, never an enclosing class body. A dataclass built on such an alias was taking its inherited fields from a module-level namesake rather than the class the alias really names.
