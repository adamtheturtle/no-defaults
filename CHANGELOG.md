...
- An alias such as `Alias = Base` written in a class body resolves the name the way Python does: against the enclosing functions and then the module, never against an outer class body, which is not a closure scope. A dataclass inheriting through such an alias was being given another class's fields.
- `import package.module` makes `package.module.Base` the submodule's class, over a same-named class the package initializer defines. The initializer's class was winning, and inherited calls were rewritten with defaults the real base never had.
