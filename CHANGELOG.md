...
- `import pkg.module` reaches the submodule even when `pkg/__init__.py` writes a class of that name. The import puts the submodule on the package in place of it, so a base spelled `pkg.module.Base` is the submodule's; the initializer's nested class was being read instead and calls were rewritten with an unrelated class's defaults.
