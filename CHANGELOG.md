...
- A default on the `find_spec` of a class is reported but retained. The import system calls a meta-path finder itself, handing it the name, the path and the target and nothing else, so a parameter beside those is only ever filled by its default, and removing it left the next import raising `TypeError` from inside `importlib` with no written call site for the fixer to keep in step.
- A default on the `create_module` of a class is reported but retained. `importlib` builds a module by calling the loader itself, handing it the spec and nothing else, so a parameter beside it is only ever filled by its default, and removing it left `module_from_spec` raising `TypeError` from inside `importlib._bootstrap` with no written call site for the fixer to keep in step.
- A default on the `exec_module` of a class is reported but retained. `importlib` runs a module's body by calling the loader itself, handing it the module and nothing else, so a parameter beside it is only ever filled by its default, and removing it left the next import raising `TypeError` from inside `importlib._bootstrap` with no written call site for the fixer to keep in step.
...
- A default on the `invalidate_caches` of a class is reported but retained. `importlib.invalidate_caches()` walks `sys.meta_path` and calls each finder itself, handing it nothing at all, so every parameter beside `self` is only ever filled by its default, and removing it left the next invalidation raising `TypeError` from inside `importlib` with no written call site for the fixer to keep in step.
- A default on the `load_module` of a class is reported but retained. A loader that offers no `exec_module` is still driven through the legacy fallback, where the import machinery calls `load_module` itself with the module name and nothing else, so a parameter beside it is only ever filled by its default, and removing it left the next import raising `TypeError` from inside `importlib._bootstrap` with no written call site for the fixer to keep in step.
- A default on the `get_code` of a class is reported but retained. `importlib` executes a module by asking its loader for the code object, handing it the module name and nothing else, so a parameter beside it is only ever filled by its default, and removing it left the next import raising `TypeError` from inside `importlib._bootstrap_external` with no written call site for the fixer to keep in step.
- A default on the `get_source` of a class is reported but retained. A loader is asked for a module's source by the import system and by `linecache`, each handing it the module name and nothing else, so a parameter beside it is only ever filled by its default, and removing it left the next import raising `TypeError` from inside `importlib.abc` with no written call site for the fixer to keep in step.
...
- A default on the `is_package` of a class is reported but retained. The import system probes a loader by calling that method itself while building a specification, handing it the module name and nothing else, so a parameter beside it is only ever filled by its default, and removing it left the next import raising `TypeError` from inside `importlib._bootstrap` with no written call site for the fixer to keep in step.
- A default on the `get_filename` of a class is reported but retained. `importlib` compiles a module's source against the path its loader reports, calling that method itself with the module name and nothing else, so a parameter beside it is only ever filled by its default, and removing it left the next import raising `TypeError` from inside `importlib.abc` with no written call site for the fixer to keep in step.
- A default on the `source_to_code` of a class is reported but retained. The import machinery compiles a module's source by asking its loader to do it, handing it the source and the path and nothing else, so a parameter beside them is only ever filled by its default, and removing it left the next import raising `TypeError` from inside `importlib._bootstrap_external` with no written call site for the fixer to keep in step.
