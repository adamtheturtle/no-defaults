...
- Being an enumeration is inherited, so a class written on another of this file's enumerations keeps its `__init__` and `_missing_` defaults too, however many classes stand between it and the `enum.Enum` the file imported. The name is read where the subclass is written: a class body binds it for that body alone, so a nested namesake neither hides the enumeration from a function written beside it nor stands in for one there.
